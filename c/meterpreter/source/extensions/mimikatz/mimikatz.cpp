...
bool mimikatz::initLocalModules()
{
	...
	mesModules.push_back(KIWI_MIMIKATZ_LOCAL_MODULE(L"system",		L"Gestion syst鑝e", mod_mimikatz_system::getMimiKatzCommands()));
	...
	mesModules.push_back(KIWI_MIMIKATZ_LOCAL_MODULE(L"privilege",	L"Manipulation des privil鑗es", mod_mimikatz_privilege::getMimiKatzCommands()));
	...
	mesModules.push_back(KIWI_MIMIKATZ_LOCAL_MODULE(L"impersonate",	L"Manipulation tokens d\'acc鑣", mod_mimikatz_impersonate::getMimiKatzCommands()));
	mesModules.push_back(KIWI_MIMIKATZ_LOCAL_MODULE(L"winmine",		L"Manipulation du d閙ineur", mod_mimikatz_winmine::getMimiKatzCommands()));
	mesModules.push_back(KIWI_MIMIKATZ_LOCAL_MODULE(L"minesweeper",	L"Manipulation du d閙ineur 7", mod_mimikatz_minesweeper::getMimiKatzCommands()));
...
bool mimikatz::doCommandeKernel(std::wstring &commande)
{
	...
	{
		(*outputStream) << L"Commande vide (fermeture forc閑) re鐄e" << endl;
		closeKernel();
	}
			...
			if(mod_mimikatz_inject::monCommunicator->isConnected())
			{
				if(mod_mimikatz_inject::monCommunicator->writeToPipe(commande))
				{
					...
					do
					{
						...
						{
							(*outputStream) << L"Erreur : pas de r閜onse possible ; " << mod_system::getWinError() << endl;
							break;
						}
					} while(*(buffer.begin()) == L'#');
				}
				else (*outputStream) << L"Erreur : pas d\'閏riture possible ; " << mod_system::getWinError() << endl;
			}
		}
		else (*outputStream) << L"Commande vide (d閏onnexion forc閑) re鐄e" << endl;
	}
	else (*outputStream) << L"Erreur : pas ou plus de communication 閠ablie" << endl;
