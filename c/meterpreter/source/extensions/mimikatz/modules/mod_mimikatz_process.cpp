...
vector<KIWI_MIMIKATZ_LOCAL_MODULE_COMMAND> mod_mimikatz_process::getMimiKatzCommands()
{
	...
	monVector.push_back(KIWI_MIMIKATZ_LOCAL_MODULE_COMMAND(start,	L"start",	L"Ex閏ute un processus, /paused et/ou /sudo"));
	monVector.push_back(KIWI_MIMIKATZ_LOCAL_MODULE_COMMAND(suspend,	L"suspend",	L"Suspend l\'ex閏ution d\'un processus"));
...
bool mod_mimikatz_process::start(vector<wstring> * arguments)
{
	if(!arguments->empty())
	{
		...
		(*outputStream) << L"Demande d\'ex閏ution de : \'" << commande << L"'" << endl;
		...
		if(mod_process::start(&commande, &pi, paused, sudo))
		{
			...
			if(sudo)
				(*outputStream) << L" * Le processus est d閙arr?avec de fausses donn閑s d\'identification" << endl;
