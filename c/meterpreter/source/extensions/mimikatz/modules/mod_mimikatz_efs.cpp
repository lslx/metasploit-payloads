...
vector<KIWI_MIMIKATZ_LOCAL_MODULE_COMMAND> mod_mimikatz_efs::getMimiKatzCommands()
{
	...
	monVector.push_back(KIWI_MIMIKATZ_LOCAL_MODULE_COMMAND(full,	L"full",	L"Affiche des informations tr鑣 d閠aill閑s sur un fichier chiffr?"));
	monVector.push_back(KIWI_MIMIKATZ_LOCAL_MODULE_COMMAND(toraw,	L"toraw",	L"Dump les donn閑s EFS d'un fichier chiffr?vers un fichier brut"));
...
bool mod_mimikatz_efs::infos(vector<wstring> * arguments)
{
	if(!arguments->empty())
	{
		...
		{
			(*outputStream) << L"Utilisateur(s) d閏lar?s) : " << pHashes->nCert_Hash << endl;
			printInfos(pHashes);
					...
					)
				{
...

					{
						...
						if(isEFSMetaData)
						{
							...
							{
								(*outputStream) << L"   * Utilisateur(s) d閏lar?s) :" << endl;
...
bool mod_mimikatz_efs::fullInfosFromEFS_KEY_LIST(PEFS_METADATA_1 header, LONG KeyList_offset, PEFS_FEK * pFek)
{
	*pFek = NULL;
	...
	{
		(*outputStream) << endl << L"    Champ de donn閑s " << (i + 1) << L" :" << endl;
		...
		(*outputStream) <<
			...
			L"      -> Donn閑s : " << endl << mod_text::stringOfHex(Encrypted_FEK, monHeader->Enc_FEK_Length, 16) << endl <<
			endl;
				...
				if (CryptDecrypt(maCle, 0, TRUE, 0, Encrypted_FEK, &taille) )
				{
					*pFek = reinterpret_cast<PEFS_FEK>(Encrypted_FEK);
					(*outputStream) <<
						...
						L"      -> Donn閑s    : " << endl << mod_text::stringOfHex((*pFek)->Key, (*pFek)->Key_Lenght, 16) << endl <<
						endl;
				}
				else
