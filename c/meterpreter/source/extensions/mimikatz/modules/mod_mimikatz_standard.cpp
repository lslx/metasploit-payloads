...
vector<KIWI_MIMIKATZ_LOCAL_MODULE_COMMAND> mod_mimikatz_standard::getMimiKatzCommands()
{
	...
	monVector.push_back(KIWI_MIMIKATZ_LOCAL_MODULE_COMMAND(clearScreen,	L"cls",		L"Efface l\'閏ran (ne fonctionne pas en 閤ecution distante, via PsExec par exemple)"));
	monVector.push_back(KIWI_MIMIKATZ_LOCAL_MODULE_COMMAND(exit,		L"exit",	L"Quitte MimiKatz"));
	monVector.push_back(KIWI_MIMIKATZ_LOCAL_MODULE_COMMAND(reponse,		L"reponse",	L"Calcule la r閜onse ?la Grande Question sur la Vie, l\'Univers et le Reste"));
	...
	//monVector.push_back(KIWI_MIMIKATZ_LOCAL_MODULE_COMMAND(test,		L"test",	L"Routine de test (ne devrait plus 阾re l?en release..."));
	return monVector;
}
...
{
	(*outputStream) << L"La r閜onse est 42." << endl;
	return true;
}
